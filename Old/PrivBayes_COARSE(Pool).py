...
def Classification(data,repetitions,NumOfClasses,testing_data):
...
def PrivBayes(dataset_choice,M,split_choice,k,str_choice,AddNoise,epsilon,AddNoise1,epsilon1,AddNoise2,epsilon2,classfication_choice,repetitions,Iter_cnt,data,testing_data,Attr_domains,Child_Parents,NumOfAttrs,attributes,Results):

    Classif_Lock = thr.RLock()
    ...
    if (classfication_choice == 1):
        ...
        logging.info ("Please wait. This may take a few minutes.....")

        Classif_Lock.acquire()
        try:
            #f = open("Outputs/Results.txt","a")
            #f.write("################################################################\n")
            #f.write("Iteration = " + str(Iter_cnt) + ", Dataset = " + str(dataset_choice) + ", Users = " + str(M) + ", Split Choice = " + str(split_choice) + ", Str Learning = " + str(str_choice) + ", Network Degree = " + str(k)  + ", AddNoise = " + str(AddNoise) + ", Epsilon = " + str(epsilon) + ", AddNoise1 = " + str(AddNoise1) + ", Epsilon1 = " + str(epsilon1) + ", AddNoise2 = " + str(AddNoise2) + ", Epsilon2 = " + str(epsilon2) + "\n")
            #f.close()

            f1 = open("Outputs/Results.csv","a")
            f1.write(str(Iter_cnt) + "," + str(dataset_choice) + "," + str(M) + "," + str(split_choice) + "," + str(str_choice) + "," + str(k)  + "," + str(AddNoise) + "," + str(epsilon) + "," + str(AddNoise1) + "," + str(epsilon1) + "," + str(AddNoise2) + "," + str(epsilon2) + ",")
            ...
            Results = Classification(Synth_Dataset,repetitions,NumOfClasses,testing_data[:])
            ...
            Classif_Lock.release() # release lock, no matter what

        logging.info("Finished classification of synthetic data!")

    logging.info("The program is complete!!")
    print(str(multiprocessing.current_process()) + " is finished!")
                            ...
                            for str_choice in range(1,4):
                                for epsilon in epsilon_vals:
                                    ...
                                    if (cnt in Completed_Outputs):
                                        ...
                                        Cases.append((dataset_choice, M, split_choice,k,str_choice,AddNoise,epsilon,AddNoise1,epsilon1,AddNoise2,epsilon2,classfication_choice,repetitions,cnt,data,testing_data,Attr_domains,Child_Parents,NumOfAttrs,attributes,Results))
