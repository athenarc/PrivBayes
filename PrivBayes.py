...
def GetCounts(Attributes,Dataset,dA_sub,Attribute_dict,AddNoise,StrL,epsilon,k): # This is a function that each user runs on his own dataset from a dataset (returns counts cA of dataset)
    ...
    if (AddNoise):
        ...
        Counts = AddListsOfNumbers(Counts,noise) # Add noise to count to ensure e-differential privacy
...
def GetVotes(Dataset,A,k,Attr_domains,V,Datasets_Probabilities,Dataset_idx,AddNoise,epsilon_1): # This is a function that each user runs on his own dataset (Returns Child,Parent pair with the highest mutual information)
    ...
    if (Datasets_Probabilities[Dataset_idx] == 0): # We need to calculate the probabilities
        ...
        for A_sub in A_subs:  # Sharing the Noisy Sufficient Statistics line 2
            ...
            # Calculate probability PAi for each Ai (Line 5)
...
def PrivBayes(dataset_choice,M,split_choice,k,str_choice,AddNoise,epsilon,AddNoise1,epsilon1,AddNoise2,epsilon2,classfication_choice,repetitions,Iter_cnt,data,testing_data,Attr_domains,Child_Parents,NumOfAttrs,attributes,Datasets):
    ...
    if (str_choice == 1):
        ...
        Start = time.time()
        ##print ("Please wait. This may take a few minutes.....")
        SharingNoisyModel(Datasets,k,attributes[:],Attr_domains,AddNoise1,epsilon1,AddNoise2,epsilon2,Iter_cnt)
    ...
    if (classfication_choice == 1):
        ...
        logging.info("Starting classification of synthetic data")
        ##print ("Please wait. This may take a few minutes.....")
        ...
        print("Duration of classification:",time.time() - start)

        #BayesNetClassification(Conditional_Probabilities,testing_data,Attribute_dict,Attr_domains)
        ...
        FinalSynthDataset = np.array(FinalSynthDataset).astype('float')

        #BayesNetClassification(Conditional_Probabilities,FinalSynthDataset,Attribute_dict,Attr_domains)
                                        ...
                                        if (AddNoise == True):
                                            for epsilon in epsilon_vals:
                                                ...
                                                if (cnt in Completed_Outputs):
                                                    ...
                                                    PrivBayes(dataset_choice,M,split_choice,k,str_choice,AddNoise,epsilon,AddNoise1,epsilon1,AddNoise2,epsilon2,classfication_choice,repetitions,cnt,data,testing_data,Attr_domains,Child_Parents,NumOfAttrs,attributes,Datasets)
                                                    f1 = open("Outputs/Results.csv","a")
                                                    f1.write(str(cnt) + "," + str(dataset_choice) + "," + str(M) + "," + str(split_choice) + "," + str(str_choice) + "," + str(k)  + "," + str(AddNoise) + "," + str(epsilon) + "," + str(AddNoise1) + "," + str(epsilon1) + "," + str(AddNoise2) + "," + str(epsilon2) + ",")
                                                    f1.close()
                                        else:
                                            if (cnt in Completed_Outputs):
                                                pass
                                            else:
                                                PrivBayes(dataset_choice,M,split_choice,k,str_choice,AddNoise,0,AddNoise,0,AddNoise,0,classfication_choice,repetitions,cnt,data,testing_data,Attr_domains,Child_Parents,NumOfAttrs,attributes,Datasets)
                                                f1 = open("Outputs/Results.csv","a")
                                                f1.write(str(cnt) + "," + str(dataset_choice) + "," + str(M) + "," + str(split_choice) + "," + str(str_choice) + "," + str(k)  + "," + str(AddNoise) + "," + str(0) + "," + str(AddNoise) + "," + str(0) + "," + str(AddNoise) + "," + str(0) + ",")
                                                f1.close()
